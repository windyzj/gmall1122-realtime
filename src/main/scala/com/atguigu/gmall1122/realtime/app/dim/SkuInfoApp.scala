...
import com.atguigu.gmall1122.realtime.util.{MyKafkaUtil, OffsetManager, PhoenixUtil}
...
object SkuInfoApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val skuInfoDstream: DStream[SkuInfo] = objectDstream.transform { rdd =>
      ...
      if(rdd.count()>0){
        //category3
        val category3Sql  = "select id ,name   from gmall1122_base_category3" //driver  周期性执行
        ...
        // spu
        val spuSql = "select id ,spu_name  from gmall1122_spu_info" // spu
